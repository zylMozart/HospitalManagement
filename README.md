...
接化发医院监护系统的开发隶属于一般性软件项目开发工作，其主要涉及的开发成本来源于前期项目准备、软件设计、编码实现以及产品测试人员的人力成本，其所有使用的开发软件、开发框架均属于开源性质。

##### 1.4.2运行费用

该项目的运行费用主要来自两个方面，后期系统运营费用和网站服务器的租赁费用。
后期系统运营人员的工作主要涉及定期的数据备份、网站安全的维护以及对使用者的基本培训等方面，是项目正常运行必不可少的一环。按当前市场行情来看，考虑到地区之间的差异性，维护与本项目相类似的小型网站的月工资大概在2500-5000人民币左右。由于系统中许多传感器要定期更新等其他需求，产品更新及折旧费用一年在1000000人名币左右。
网站服务器的租赁费用属于基本硬件的开销。针对一家医院来讲，与一般网站访问人群基数相比，其每日患者等承载量较大，需要的服务器配置需定位响应较高。因此，该项目运行所需服务器的性能要求较高。按当前的市场价位估计，该项目每年在服务器上的开销大约在8000元左右。

##### 1.4.3经济效益

某医院目前由于完全采用纯人工的方式来完成工作的，医务人员要一边关注某些病人的情况，一边还要忙着对其它的病人进行医疗诊断，工作量大，耗时比较多，所以工作效率低。而如果开发出一个能满足业务要求的医疗计算机监护系统，由于系统能在未来较长的一段时间内稳定地发挥作用。

根据月前医院内部员工的日人工成本为: x人* y元/人=z元。我们还不能计算出因效率低下而给医院来的无形经济损失，如果指导这一部分也看作是成本，那将远远超出目前的计算数额。而如果开发出一一个能满足业务要求的医疗计算机监护系统，在采用生命周期的前提下，从问题识别到系统实施、评价、维护，开发周期如果以两年计，共需人工成本m元，各种软硬件成本n元，日常维护费用0元，共计成本费用p元，略高/低于两年的人工费用总和。同样，我们也无法估计算出则于系统的开发应用使医院运营效率提高而带来的无形的巨额经济效益，由于系统能在未来较长的一段时间内稳定地发挥作用，这对于医院的提高管理水平很大的帮助，才能使A医院早日接入到总行的更高层次的网络体系中，可以更加广泛的吸收各方面的信息资源，可为医院业务在将来的扩张打下坚实的基础，其经济效益将更上一层楼。
...
由于系统主要功能在可行性分析阶段已经进行了严谨的分析讨论，因此在该阶段可以直接沿用之前的顶层数据流图。该数据流图以人机交互思想为指导，从整体观念出发，展示了流入和流出系统的所有数据信息。
...
管理员可以查看所有患者/护士/医生的信息。![21.1](.\img\UI\21.1.png)

管理员可以查看用户的详细信息，同时，该界面不能编辑数据。![21.2](.\img\UI\21.2.png)

管理员可以修改用户的信息，在提交之后，前端数据会跟着同步信息。

![21.3](.\img\UI\21.3.png)

管理员可以添加用户信息，单击添加按钮后可以添加新的用户，并且有消息提示添加成功/添加失败。

![21.4](.\img\UI\21.4.png)

![21.5](.\img\UI\21.5.png)

管理员可以删除用户信息，单击按钮后可以删除用户，并且有消息提示删除成功/删除失败。

![21.6](.\img\UI\21.6.png)

#### 5.5测试

##### 5.2.1 软件测试的目标

对于软件测试来说，可以从两个方向来理解。正向思维出发点：使自己确信产品是能够正常工作的评价一个程序和系统的特性或能力，并确定它是否达到期望的结果，软件测试就是以此为目的的任何行为。反向思维：测试是为发现错误而执行一个程序或者系统的过程，测试是为了证明程序有错，而不是证明程序无错误，一个好的测试用例在于它能发现以前未发现的错误，一个成功的测试是发现了以前未发现的错误的测试。那么通过对软件测试的描述，其实能总结出软件测试的目的，以最少的人力、物力和时间找出软件中潜在的各种错误和缺陷，通过修正各种错误和缺陷保障软件质量，避免软件发布后由于潜在的软件错误和缺陷造成的隐患所带来的商业风险。同时利用测试过程中得到的测试结果和测试信息，作为后续项目开发和测试过程改进的重要输入，避免在将来的项目开发和测试中重复同样的错误；采用更加高效的测试管理手段，提高软件测试的效率和软件产品的质量。
...
| 输入等价类 | 有效等价类                                                   | 无效等价类                                                   |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 注册信息   | 1.用户类型、用户ID和用户密码不为空值，且所有数据与数据库中的信息相匹配 | 1.无效的用户类型<br />2.无效的用户ID<br />3.用户ID与用户密码不一致<br />4.存在空值 |

有效等价类测试用例表

| 测试数据                                                   | 期望结果 | 覆盖的有效等价类 |
| ---------------------------------------------------------- | -------- | ---------------- |
| 用户类型：patient<br />用户ID：101<br />用户密码：12345678 | 有效输入 | 1                |

无效等价类测试用例表

| 测试数据                                                     | 期望结果 | 覆盖的无效等价类 |
| ------------------------------------------------------------ | -------- | ---------------- |
| 用户类型：kakaka<br />用户ID：101<br />用户密码：12345678    | 无效输入 | 1                |
| 用户类型：patient<br />用户ID：666<br />用户密码：12345678   | 无效输入 | 2                |
| 用户类型：patient<br />用户ID：101<br />用户密码：1234567890 | 无效输入 | 3                |
| 用户类型：null<br />用户ID：101<br />用户密码：12345678      | 无效输入 | 4                |
...
| 输入等价类 | 有效等价类                   | 无效等价类                         |
| ---------- | ---------------------------- | ---------------------------------- |
| 诊断信息   | 1.用户ID存在，诊断信息不为空 | 1.用户ID不存在<br />2.诊断信息为空 |

有效等价类测试用例表

| 测试数据                                      | 期望结果 | 覆盖的有效等价类 |
| --------------------------------------------- | -------- | ---------------- |
| 用户ID：101<br />诊断内容：考试不好，需要重修 | 有效输入 | 1                |

无效等价类测试用例表

| 测试数据                                        | 期望结果 | 覆盖的无效等价类 |
| ----------------------------------------------- | -------- | ---------------- |
| 用户ID：10001<br />诊断内容：考试不好，需要重修 | 无效输入 | 1                |
| 用户ID：101<br />诊断内容：null                 | 无效输入 | 2                |
...
| 输入等价类                 | 有效等价类                                       | 无效等价类                                         |
| -------------------------- | ------------------------------------------------ | -------------------------------------------------- |
| 报警标准<br />病人生理特征 | 1.报警标准存在<br />2.病人生理特征落符合报警标准 | 1.报警标准不存在<br />2.病人生理特征不在报警标准内 |

有效等价类测试用例表

| 测试数据                                                     | 期望结果 | 覆盖的有效等价类 |
| ------------------------------------------------------------ | -------- | ---------------- |
| 报警标准：血压过高、心跳骤停...<br />病人ID：101<br />病人生理特征：血压200，心跳0 | 有效输入 | 1                |

无效等价类测试用例表

| 测试数据                                                     | 期望结果 | 覆盖的无效等价类 |
| ------------------------------------------------------------ | -------- | ---------------- |
| 报警标准：null<br />病人ID：101<br />病人生理特征：血压200，心跳0 | 无效输入 | 1                |
| 报警标准：血压过高、心跳骤停...<br />病人ID：101<br />病人生理特征：血压100，心跳60 | 无效输入 | 2                |
...
| 输入等价类   | 有效等价类                                             | 无效等价类                                   |
| ------------ | ------------------------------------------------------ | -------------------------------------------- |
| 查询用户信息 | 1.用户ID存在                                           | 1.用户ID不存在                               |
| 修改用户信息 | 2.用户ID存在，修改后的内容符合标准                     | 2.用户ID不存在<br />3.修改后的内容不符合标准 |
| 添加用户信息 | 3.新增的用户ID不存在于原数据库中，修改后的内容符合标准 | 4.用户ID存在<br />5.添加后的内容不符合标准   |
| 删除用户信息 | 4.用户ID存在                                           | 6.用户ID不存在                               |

有效等价类测试用例表

| 测试数据（查询模块）                                         | 期望结果     | 覆盖的有效等价类     |
| ------------------------------------------------------------ | ------------ | -------------------- |
| 用户ID：101                                                  | 有效输入     | 1                    |
| **测试数据（修改模块）**                                     | **期望结果** | **覆盖的有效等价类** |
| 用户ID：101<br />用户类别：患者<br />用户姓名：西施<br />用户性别：女<br />用户年龄：18<br />用户地址：西溪医院<br />用户密码：12345678 | 有效输入     | 2                    |
| **测试数据（添加模块）**                                     | **期望结果** | **覆盖的有效等价类** |
| 用户ID：109<br />用户类别：患者<br />用户姓名：西施<br />用户性别：女<br />用户年龄：18<br />用户地址：西溪医院<br />用户密码：12345678 | 有效输入     | 3                    |
| **测试数据（删除模块）**                                     | **期望结果** | **覆盖的有效等价类** |
| 用户ID：101                                                  | 有效输入     | 4                    |
...
| 用户ID：1999                                                 | 无效输入     | 1                    |
| **测试数据（修改模块）**                                     | **期望结果** | **覆盖的有效等价类** |
| 用户ID：1999<br />用户类别：患者<br />用户姓名：西施<br />用户性别：女<br />用户年龄：18<br />用户地址：西溪医院<br />用户密码：12345678 | 无效输入     | 2                    |
| 用户ID：101<br />用户类别：患者<br />用户姓名：西施<br />用户性别：不知道<br />用户年龄：-10<br />用户地址：西溪医院<br />用户密码：12345678 | 无效输入     | 3                    |
| **测试数据（添加模块）**                                     | **期望结果** | **覆盖的有效等价类** |
| 用户ID：101<br />用户类别：患者<br />用户姓名：西施<br />用户性别：女<br />用户年龄：18<br />用户地址：西溪医院<br />用户密码：12345678 | 无效输入     | 4                    |
| 用户ID：101<br />用户类别：患者<br />用户姓名：西施<br />用户性别：不知道<br />用户年龄：-10<br />用户地址：西溪医院<br />用户密码：12345678 | 无效输入     | 5                    |
| **测试数据（删除模块）**                                     | **期望结果** | **覆盖的有效等价类** |
| 用户ID：19999                                                | 无效输入     | 6                    |
...
系统测试是对整个系统的测试，将硬件、软件、操作人员看作一个整体，检验它是否有不符合系统说明书的地方。这种测试可以发现系统分析和设计中的错误。如安全测试是测试安全措施是否完善，能不能保证系统不受非法侵入。再例如，压力测试是测试系统在正常数据量以及超负荷量(如多个用户同时存取) 等情况下是否还能正常地工作。比较常见的、典型的系统测试包括恢复测试、安全测试、压力测试。
...
回归测试是指修改了旧代码后，重新进行测试以确认修改没有引入新的错误或导致其他代码产生错误。自动回归测试将大幅降低系统测试、维护升级等阶段的成本。回归测试作为软件生命周期的一个组成部分，在整个软件测试过程中占有很大的工作量比重，软件开发的各个阶段都会进行多次回归测试。在渐进和快速迭代开发中，新版本的连续发布使回归测试进行的更加频繁，而在极端编程方法中，更是要求每天都进行若干次回归测试。因此，通过选择正确的回归测试策略来改进回归测试的效率和有效性是很有意义的。选择回归测试策略应该兼顾效率和有效性两个方面。常用的选择回归测试的方式有一下几类：
...
当测试者对修改的局部化有足够的信心时，可以通过相依性分析识别软件的修改情况并分析修改的影响，将回归测试局限于被改变的模块和它的接口上。通常，一个回归错误一定涉及一个新的、修改的或删除的代码段。在允许的条件下，回归测试尽可能覆盖受到影响的部分。再测试全部用例的策略是最安全的策略，但已经运行过许多次的回归测试不太可能揭示新的错误，而且很多时候，由于时间、人员、设备和经费的原因，不允许选择再测试全部用例的回归测试策略，此时，可以选择适当的策略进行缩减的回归测试。

###### 5.3.3.4系统测试结论

经过对该系统的需求分析的要求进行基本功能测试后，基本功能可以正常使用，数据库功能也正常，系统四种类型的用户互不干扰，数据更新未发生冲突，所以结果符合接化发医院监护系统的需求。
...
通过软件工程实验课程的训练，本团队三人在整个过程中都收获颇丰。我们小组每个成员都深刻意识到利用软件工程思想指导项目进行分析、设计、实现、测试和维护等过程中的必要性。在我们的不断努力下，克服重重困难，最终实现文档中涉及到的功能。
...
通过利用《软件工程》的系统开发模式，更加清楚了系统开发的流程，使系统开发模块化、清晰化。同时，通过这次的项目，增强了自己的动手能力及团队协作能力，十分有意义。

**刑德壮**

在学习《软件工程导论》这门课后，我发现这门课并不是我想象中那么枯燥。我了解到软件工程是一门研究用工程化方法构建和维护有效的，实用的和高质量的学科。它涉及到课程设计语言、数据库、软件开发工具、系统平台、标准、设计模型等方面。在现实生活中，可以帮助我们工作更加高效，提高我们的公作效率。